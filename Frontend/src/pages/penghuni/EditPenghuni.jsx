...
import { getPenghuniById, storageURL, updatePenghuni } from "../../services/penghuni/PenghuniServices";
...
const EditPenghuni = () => {
  ...
  const fetchPenghuniById = async () => {
    // Fetch data by ID
    try {
      const response = await getPenghuniById(id);
      setNamaLengkap(response.nama_lengkap);
      setJenisKelamin(response.jenis_kelamin);
      setNoTelepon(response.no_telepon);
      setStatusPenghuni(response.status_penghuni);
      setStatusPernikahan(response.status_pernikahan);
      setFotoKTP(response.foto_ktp);
    } catch (error) {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const response = await updatePenghuni(id, formData);
      console.log(response);
      Swal.fire({
        icon: "success",
        title: "Success",
        text: "Data penghuni berhasil diubah",
      });
      navigate("/penghuni");
    } catch (error) {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Data penghuni gagal diubah",
      });
    }
  }
    ...
    <>
      <div className="container-xxl flex-grow-1 container-p-y">
        <h4 className="fw-bold py-3 mb-4">
          <span className="text-muted fw-light">Penghuni/</span> Edit Penghuni
        </h4>
